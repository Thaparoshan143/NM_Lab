...
int main()
{
    ...
    printf("Fitting the straight line y = ae^bx using the method of least square\n");
    ...
    for(int i=1;i<=n;i++)
    {
        sumx += log(point[i].x);
        sumy += log(point[i].y);
        sumx2 += log(point[i].x )* log(point[i].x);
        sumxy += log(point[i].x) * log(point[i].y);
    }
    ...
    printf("\nThe Fitted regression line is  y = %0.9fe^(%0.9fx)", a, b);
